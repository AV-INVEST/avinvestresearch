...
const faqsByCategory: Record<CategoryKey, FAQItem[]> = {
  generali: [
    {
...
export default function FAQ() {
  const [active, setActive] = useState<CategoryKey>('generali');

  const visibleFaqs = useMemo<FAQItem[]>(
    () => faqsByCategory[active] ?? [],
    [active],
  );
  ...
  return (
    <section id="faq" className="relative py-14 sm:py-32 scroll-mt-28" aria-labelledby="faq-heading">
      <div className="container-page">
        ...
        <div className="mx-auto mt-7 sm:mt-10 max-w-3xl w-full min-w-0">
          <div className="w-full min-w-0 max-w-full overflow-hidden sm:contents">
            <div
              className="w-full min-w-0 overflow-x-auto scrollbar-hidden flex gap-2 pb-3 sm:overflow-visible sm:pb-0 sm:flex-wrap sm:justify-center"
              role="tablist"
              ...
              {categories.map((cat) => {
                ...
                return (
                  <button
                    ...
                    aria-controls={`faq-panel-${cat.key}`}
                    id={`faq-tab-${cat.key}`}
                    onClick={() => setActive(cat.key)}
                    className={`inline-flex shrink-0 whitespace-nowrap items-center gap-1.5 rounded-full border px-4 py-2 text-xs font-semibold uppercase tracking-wider transition-all duration-200 focus-visible:outline-av-green ${
                      isActive
                        ? 'border-av-green-deep/60 bg-av-green/12 text-av-green shadow-glow-green-sm'
                        : 'border-av-line bg-av-bg-2/70 text-av-muted hover:border-av-green-deep/40 hover:text-white hover:bg-av-surface'
                    }`}
                  >
                    {cat.label}
                    <span
                      className={`ml-1 rounded-full px-1.5 py-0.5 text-[10px] font-bold tabular-nums ${
                        isActive
                          ? 'bg-av-green/25 text-av-green'
                          : 'bg-av-line text-av-muted'
                      }`}
                      aria-hidden="true"
                    >
